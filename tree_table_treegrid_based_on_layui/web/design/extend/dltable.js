...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function(exports){
    ...
    var $ = layui.$
        ...
        ,TREE_ID='treeId',TREE_UPID='treeUpId',TREE_SHOW_NAME='treeShowName',TREE_KEY_MAP='tree_key_map',TREE_SHOWFLAG='treeShowFlag'
        //thead区域模板
     ...
     * @returns {*}
     */
    var tds_fixed = [], tds_fixed_r = [];
    Class.prototype.renderTr=function (obj,numbers,a) {
        var that = this
            ,options = that.config;
        var tds= [];
        that.eachCols(function(i3, cols){//cols列定义
            ...
            tds.push(td);
            if(cols.fixed && cols.fixed !== 'right'&& a==1) {
                console.log(numbers)
                tds_fixed.push(td);
            }
            if(cols.fixed === 'right'&& a==1) {
                console.log(numbers)
                tds_fixed_r.push(td);
            }
        });
    ...
    Class.prototype.renderData = function(res, curr, count, sort){
        var that = this
            ...
            ,render = function(){ //后续性能提升的重点
                ...
                layui.each(data, function(i1, obj){
                    ...
                    tds=that.renderTr(obj,numbers,a);
                    a++;
        ...
        for(let key  in data){
            debugger
            if (options.treeShowFlag==false){
                that.treeNodeOpen(data[key],false);
            }else {
                that.treeNodeOpen(data[key],true);
            }
